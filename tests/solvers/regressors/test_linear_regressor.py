...
from gumps.solvers.regressors.linear_regressor import (
    LinearRegressionParameters, LinearRegressor)
...
def generate_regression_parameters(count: int = 5):
    ...
    return LinearRegressionParameters(input_data=input_data, output_data=output_data)

class TestLinearRegressionSolver(unittest.TestCase):
    "test the regressor"

    def test_linear_parameters_ok(self):
        "test the polynomial parameters"
        count = 5
        input_data = pd.DataFrame({"x": np.linspace(0, 1, count)})
        output_data = pd.DataFrame({"y": np.linspace(0, 1, count)})
        LinearRegressionParameters(input_data=input_data, output_data=output_data)

    def test_linear_parameters_ok_set(self):
        "test the polynomial parameters"
        count = 5
        input_data = pd.DataFrame({"x": np.linspace(0, 1, count)})
        output_data = pd.DataFrame({"y": np.linspace(0, 1, count)})
        LinearRegressionParameters(input_data=input_data, output_data=output_data, order = 1)

    def test_linear_parameters_bad(self):
        "test the polynomial parameters"
        count  = 5
        input_data = pd.DataFrame({"x": np.linspace(0, 1, count)})
        output_data = pd.DataFrame({"y": np.linspace(0, 1, count)})
        ...
        with self.assertRaises(ValueError):
            LinearRegressionParameters(input_data=input_data, output_data=output_data, order=order)

    def test_initialize(self):
        "test the initialization"
        parameters = generate_regression_parameters()
        regressor = LinearRegressor(parameters)
        self.assertIsInstance(regressor.parameters, LinearRegressionParameters)
    ...
    def test_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir = Path(temp_dir)
            regressor.save(temp_dir)
            new_regressor = LinearRegressor.load(temp_dir)
    ...
    def test_load_rebuild(self):
        ...
        regressor_dir = base_dir / "regressor" / "linear"
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(TestLinearRegressionSolver)
