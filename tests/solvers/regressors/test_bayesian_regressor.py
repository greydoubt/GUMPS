...
from gumps.solvers.regressors.bayesian_regressor import (
    BayesianRegressionParameters, BayesianRegressor)
...
def generate_regression_parameters(count: int = 5):
    ...
    order = 3
    return BayesianRegressionParameters(input_data=input_data,
...
class TestBayesianRegressionSolver(unittest.TestCase):
    "test the regressor"
    ...
    def test_polynomial_parameters_ok(self):
        ...
        output_data = pd.DataFrame({"y": np.linspace(0, 1, count)})
        order = 2
        BayesianRegressionParameters(input_data=input_data,
                                     output_data=output_data, order=order)
    ...
    def test_polynomial_parameters_logging(self):
        ...
        with self.assertLogs('gumps.solvers.regressors.polynomial_regressor', level='INFO') as cm:
            BayesianRegressionParameters(input_data=input_data, output_data=output_data, order=order)
    ...
    def test_initialize(self):
        ...
        regressor = BayesianRegressor(parameters)
        self.assertIsInstance(regressor.parameters, BayesianRegressionParameters)
        ...
        self.assertIsInstance(regressor.regressor, MultiOutputRegressor)
    ...
    def test_get_regressor(self):
        ...
        self.assertIsInstance(reg, MultiOutputRegressor)
    ...
    def test_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir = Path(temp_dir)
            regressor.save(temp_dir)
            new_regressor = BayesianRegressor.load(temp_dir)
    ...
    def test_config(self):
        ...
        regressor = BayesianRegressor(parameters)
        regressor.fit()
        for reg in regressor.regressor.estimators_:
            self.assertTrue(reg.n_iter_ == 1)
    ...
    def test_load_rebuild(self):
        ...
        regressor_dir = base_dir / "regressor" / "bayesian"
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(TestBayesianRegressionSolver)
