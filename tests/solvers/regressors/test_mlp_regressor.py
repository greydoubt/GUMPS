...
"test the multi layer perceptron regressor"
...
from gumps.solvers.regressors.mlp_regressor import (
    MultiLayerPerceptronRegressionParameters, MultiLayerPerceptronRegressor)
...
def generate_regression_parameters(count: int = 5) -> MultiLayerPerceptronRegressionParameters:
    ...
    return MultiLayerPerceptronRegressionParameters(input_data=input_data, output_data=output_data)

class TestMultiLayerPerceptronRegressionSolver(unittest.TestCase):
    ...
    def test_initialize_kernel(self):
        ...
        regressor = MultiLayerPerceptronRegressor(parameters)
        self.assertIsInstance(regressor.parameters, MultiLayerPerceptronRegressionParameters)
        ...
        self.assertIsInstance(regressor.regressor, sklearn.neural_network.MLPRegressor)
    ...
    def test_get_regressor(self):
        ...
        self.assertIsInstance(reg, sklearn.neural_network.MLPRegressor)
    ...
    def test_get_regressor_with_parameters(self):
        ...
        reg = regressor._get_regressor()
        self.assertIsInstance(reg, sklearn.neural_network.MLPRegressor)
        self.assertEqual(reg.hidden_layer_sizes, (10, 20))

    def test_mlp_parameters_validation_fraction_less_than_zero(self):
        "test the initialization of the kernel when it is None"
        parameters = generate_regression_parameters()
    ...
    def test_mlp_parameters_validation_fraction_greater_than_one(self):
        "test the initialization of the kernel when it is None"
        parameters = generate_regression_parameters()
        with self.assertRaises(ValueError):
    ...
    def test_mlp_parameters_validation_fraction_zero(self):
        "test the initialization of the kernel when it is None"
        parameters = generate_regression_parameters()
        with self.assertRaises(ValueError):
    ...
    def test_mlp_parameters_validation_fraction_one(self):
        "test the initialization of the kernel when it is None"
        parameters = generate_regression_parameters()
        with self.assertRaises(ValueError):
    ...
    def test_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir = Path(temp_dir)
            regressor.save(temp_dir)
            new_regressor = MultiLayerPerceptronRegressor.load(temp_dir)
    ...
    def test_load_rebuild(self):
        ...
        regressor_dir = base_dir / "regressor" / "mlp"
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(TestMultiLayerPerceptronRegressionSolver)
