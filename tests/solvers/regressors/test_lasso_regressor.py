...
from gumps.solvers.regressors.lasso_regressor import (
    LassoRegressionParameters, LassoRegressor)
...
def generate_regression_parameters(count: int = 5):
    ...
    order = 3
    return LassoRegressionParameters(input_data=input_data, output_data=output_data, order=order)
...
class TestLassoRegressionSolver(unittest.TestCase):
    "test the regressor"
    ...
    def test_polynomial_parameters_ok(self):
        ...
        output_data = pd.DataFrame({"y": np.linspace(0, 1, count)})
        order = 2
        LassoRegressionParameters(input_data=input_data, output_data=output_data, order=order)
    ...
    def test_initialize(self):
        ...
        regressor = LassoRegressor(parameters)
        self.assertIsInstance(regressor.parameters, LassoRegressionParameters)
    ...
    def test_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir = Path(temp_dir)
            regressor.save(temp_dir)
            new_regressor = LassoRegressor.load(temp_dir)
    ...
    def test_load_rebuild(self):
        ...
        regressor_dir = base_dir / "regressor" / "lasso"
...
if __name__ == "__main__":
    suite = unittest.TestLoader().loadTestsFromTestCase(TestLassoRegressionSolver)
